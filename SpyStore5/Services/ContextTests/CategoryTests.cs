...
namespace Services.ContextTests
{
	...
	public class CategoryTests : IDisposable
	{
		...
		public void ShouldGetAllCategoriesOrderedByName()
		{
			...
			_db.SaveChanges();
			var cateogries = _db.Categories.OrderBy(c => c.CategoryName).ToList();
			Assert.Equal(2, _db.Categories.Count());
			Assert.Equal("Bar", cateogries[0].CategoryName);
			Assert.Equal("Foo", cateogries[1].CategoryName);
		}
		...
		[Fact]
		public void ShouldDeleteCategoryWithTimestampData()
		{
			var category = new Category { CategoryName = "Foo" };
			_db.Categories.Add(category);
			_db.SaveChanges();
